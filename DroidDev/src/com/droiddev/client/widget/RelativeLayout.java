...
public class RelativeLayout extends AbstractLayout {
	...
	HashMap<Widget, Vector<Relation>> relations;
	...
	public RelativeLayout() {
		super(TAG_NAME);
		relations = new HashMap<Widget, Vector<Relation>>();
	}
	...
	protected int closestVertical(Widget w, Widget wd, int[] dist) {
		...
		for (int i=1;i<dist.length;i++) {
			...
			if (wd == w.getParentLayout() && i == 1)
				break;
		}
		// Test for centering
		if (wd == w.getParentLayout() && Math.abs(w.getY()+(w.getHeight())/2 - (wd.getHeight()/2)) < min) {
	...
	protected int closestHorizontal(Widget w, Widget wd, int[] dist) {
		...
		// Test for centering
		if (wd == w.getParentLayout() && Math.abs(w.getX()+w.getWidth()/2 - (wd.getWidth()/2)) < min) {
	...
	public void positionWidget(Widget w) {
		...
		if (w.getParentLayout() != null) {
			modeVert = closestVertical(w, w.getParentLayout(), dist);
			if (modeVert < 2 || modeVert == dist.length) {
				closeVert = dist[0];
				closestTop = w.getParentLayout();
			}
			else {
				modeVert = 0;
			}
			modeHorz = closestHorizontal(w, w.getParentLayout(), dist);
			if (modeHorz < 2 || modeHorz == dist.length) {
				closeHorz = dist[0];
				closestLeft = w.getParentLayout();
			}
			else {
				modeHorz = 0;
			}
		...
		x = w.getX();
		y = w.getY();
		if (closestTop == w.getParentLayout()) {
			switch (modeVert) {
			case 0:
				y = 0;
		...
		if (closestLeft == w.getParentLayout()) {
			switch (modeHorz) {
			case 0:
				x = 0;
